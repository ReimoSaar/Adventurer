...
import game.*;
import game.settings.Resolution;
...
public class EnemyBot {
    ...
    private EnemyBotFeatures features;

    public EnemyBot(Character player) {
        ...
        features = new EnemyBotFeatures(player);
    }

    public ArrayList<Character> getAllEnemyBots() {
        /*ArrayList<game.characters.Character> enemyBotsList = null;
    ...
    public void renderEnemyBot(Graphics g, ImageObserver imageObserver) {
        ...
        for (Character enemyBots : getAllEnemyBots()) {
            enemyBots.draw(g2, imageObserver);
    ...
    public void update(Game game) {
        for (Character enemyBot : getAllEnemyBots()) {
            enemyBot.moveCharacter(game);
            features.addShooting(enemyBot, "bullet", game.getBullets(), 0.005f, 0.005f, 0.007f, 0.0f, false);
            enemyBot.changeCharacterImg();
            enemyBot.changeCharacterDirection();
            enemyBot.addJumping(game);
        }
    ...
    public void saveEnemyBots() {
        ...
        try {
            ...
            for (Character enemyBots : getAllEnemyBots()) {
                outputWriter.write(Double.toString(enemyBots.getRectangle().x / Resolution.screenHeight) + "\n");
                outputWriter.write(Double.toString(enemyBots.getRectangle().y / Resolution.screenHeight) + "\n");
                outputWriter.write(Float.toString(enemyBots.getStandingLocation()) + "\n");
                outputWriter.write(Float.toString((float) (enemyBots.getXVel() / Resolution.screenWidth)) + "\n");
                outputWriter.write(Float.toString((float) (enemyBots.getMovementRadius() / Resolution.screenWidth)) + "\n");
    ...
    public void loadEnemyBots(LevelStateManager levelState) {
        ...
        try {
           ...
           /* enemies.add(new game.characters.Character(getStandingImage(), null, getRightAnimation(), getLeftAnimation(),
    ...
    public void addLevel1Enemies(LevelStateManager levelState) {
        /*enemies.add(new game.characters.Character(getStandingImage(), null, enemyRightAnimation[0], enemyLeftAnimation[0],
                0.8f, 0.2f, 0.03f, 0.12f, 0.002f, 0.1f, 2));
        enemies.add(new game.characters.Character(getStandingImage(), null, enemyRightAnimation[0], enemyLeftAnimation[0],
    ...
    public void calculateStandingLocation(Player playerClass, Platform platform) {
        Character player = playerClass.getPlayer();
        float standingLocation = player.getStandingLocation();
        player.setStandingLocation(standingLocation += platform.getxVel());
        float animChangeDistance = player.getAnimChangeDistance();
        player.setStandingLocation(animChangeDistance += platform.getxVel());
